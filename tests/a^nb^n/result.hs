...
main = putStrLn (if (test_accept == []) && (test_reject == []) then "test passed!" else "test failed!")
  where accept_these = ["ab", "aabb", "aaabbb", "aaaabbbb", "aaaaabbbbb"]
        reject_these = ["a", "b", "aa", "bb", "ba", "abb", "bba", "aab", "abba", "bbaa", "baba", "abab"]
        test_accept = filter (\x -> x /= Accept) (map scan_char_sequence accept_these)
        test_reject = filter (\x -> x /= Reject) (map scan_char_sequence reject_these)
...
scan input = scan' input [] "0"
  where
    ...
    scan' ("a":input) stack "1" = scan' input ("a":stack) "1"
    scan' ("b":input) ("a": stack) "1" = scan' input stack "2"
    scan' ("b":input) ("a": stack) "2" = scan' input stack "2"
